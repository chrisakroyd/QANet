...
from src import data_aug, preprocessing as prepro, util

keys_to_remove = ['tokens', 'length', 'query', 'elmo', 'lemmas', 'context_match', 'query_match', 'orig_tokens']
...
def exact_match_features(context_tokens, query_tokens, context_lemmas, query_lemmas):
    context_lower = list(map(str.lower, context_tokens))
    query_lower = list(map(str.lower, query_tokens))

    # We remove stopwords as they will always be present in query and context making
    # this feature a less meaningful representation.
    context_exact_match = set(context_tokens) - data_aug.stopwords
    query_exact_match = set(query_tokens) - data_aug.stopwords
    context_lower_match = set(context_lower) - data_aug.stopwords
    query_lower_match = set(query_lower) - data_aug.stopwords

    # We take the intersection of the sets (words that are in both context and query in different forms).
    exact_match = context_exact_match & query_exact_match
    lower_match = context_lower_match & query_lower_match
    lemma_match = set(context_lemmas) & set(query_lemmas)

    assert len(context_tokens) == len(context_lower) == len(context_lemmas)
    assert len(query_tokens) == len(query_lower) == len(query_lemmas)

    zipped_contexts = zip(context_tokens, context_lower_match, context_lemmas)
    zipped_queries = zip(query_tokens, query_lower_match, query_lemmas)

    context_vector = create_feature_vector(exact_match, lower_match, lemma_match, zipped_contexts, len(context_tokens))
    query_vector = create_feature_vector(exact_match, lower_match, lemma_match, zipped_queries, len(query_tokens))

    return context_vector, query_vector


def create_feature_vector(exact_match, lower_match, lemmas_match, zipped_data, length):
    matches = np.zeros(shape=(length, 3), dtype=np.int32)

    for i, (token, lower_token, lemma) in enumerate(zipped_data):
        if token in exact_match:
            matches[i][0] = 1

        if lower_token in lower_match:
            matches[i][1] = 1
...
def fit_and_extract(path, tokenizer, skip_on_errors=True):
    ...
    for key, context in tqdm(contexts.items()):
        ...
        tokens, modified_tokens, lemmas = tokenizer.fit_on_texts(clean_text, error_correct=True)[-1]
    ...
    for key, query in tqdm(queries.items()):
        ...
        context_match, query_match = exact_match_features(context['orig_tokens'], tokens, context['lemmas'], lemmas)
...
def create_record(context, query):
    ...
    context_match = tf.train.Feature(int64_list=tf.train.Int64List(value=query['context_match'].reshape(-1)))
    query_match = tf.train.Feature(int64_list=tf.train.Int64List(value=query['query_match'].reshape(-1)))
