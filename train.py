...
def train(sess_config, hparams):
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        # Create and initialize the model, convert the dict placeholders to a list + get the ops from call
        qanet = models.QANet(word_matrix, character_matrix, trainable_matrix, hparams)
        qanet.init(util.inputs_as_tuple(iterator.get_next()), train=True)
        ...
        for _ in tqdm(range(global_step, hparams.train_steps + 1)):
            ...
            if hparams.runtime_data and global_step % (hparams.checkpoint_every + 1) == 0:
                ...
                answer_ids, loss, answer_start, answer_end, _ = sess.run([qanet.answer_ids, qanet.loss,
                                                                          qanet.start_pointer, qanet.end_pointer,
                                                                          qanet.train_op],
            ...
            if global_step % hparams.run_val_every == 0:
                ...
                for _ in tqdm(range(1, (len(val_answers) // hparams.batch_size + 1) + 1)):
                    answer_ids, loss, answer_start, answer_end = sess.run([qanet.answer_ids, qanet.loss,
                                                                           qanet.start_pointer, qanet.end_pointer],
